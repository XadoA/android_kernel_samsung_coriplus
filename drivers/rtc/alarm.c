...
/*[[_SHP_STMC_BSP:ming2010.fan@samsung.com 2012-5-10 [Mod] 
for alarm can't wake up when force to sleep mode or restart */
#if defined(CONFIG_MACH_ZANIN_CHN_OPEN)
	wake_lock_timeout(&alarm_rtc_wake_lock, 5 * HZ);
#else
	//wake_lock_timeout(&alarm_rtc_wake_lock, 5 * HZ);
	wake_lock_timeout(&alarm_rtc_wake_lock_irq, 5 * HZ);
#endif
/*]]_SHP_STMC_BSP:ming2010.fan@samsung.com 2012-5-10*/
}
