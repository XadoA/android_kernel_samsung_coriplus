...
static int ov = 0;
#endif //chunxu.wang@samsung.com 2012-06-27 :insert usb with over voltage, CHRGR_EVENT_USBOV is overwritten by EVENT_CHGR_DETECTION.
	...
	union power_supply_propval *propval)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
...
#if defined(CONFIG_MACH_RHEA_SS_CORIPLUS)
		{
			...
			/// temp fix. Make capacity=0 when < VERYLOWVOLTAGE, so shutdown the handset. 
#if defined(CONFIG_MACH_RHEA_SS_CORIPLUS)
			if ( propval->intval <= 2 )
			{
				while (retry_cnt <= RETRY_MAX) {
					voltage = spa_get_batt_voltage_extern();
					pr_chrgr(ERROR,"%s, Forced shutdown=%d, volt==%d, retry_cnt=%d\n",
						__func__, forced_shutdown, (int)voltage, retry_cnt);
				...
				if(forced_shutdown == 1) {
					pr_chrgr(ERROR, "%s, Forced shut down! volt==%d, retry_cnt=%d\n",
							__func__, (int)voltage, retry_cnt);
					propval->intval = 0;   
				}		
			}
...
static void bcmpmu_chrgr_spa_evt(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s event=%d, data=%d\n",
		...
		if(event ==BCMPMU_CHRGR_EVENT_CHGR_DETECTION)
			pchrgr_on = (data != POWER_SUPPLY_TYPE_BATTERY)?true:false;
