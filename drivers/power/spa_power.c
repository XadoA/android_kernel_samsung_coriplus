...
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT)
static unsigned int ovp_charger_type = 0;
#endif
...
static unsigned int spa_started_fake=0;
...
{
	int fake_capacity=spa_power_iter->batt_info.fake_capacity;
...
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT)
static void spa_ovp_handle_work(void)
{
	struct spa_power_desc *spa_power_iter = g_spa_power;
	if(spa_power_iter == NULL)
	{
		pr_spa_dbg(LEVEL1, "%s : event has come before init.\n", __func__);
		return;
	}
	spa_power_iter->charger_info.charger_type = ovp_charger_type;
...
int spa_event_handler(int evt, void *data)
{
	...
	switch(evt)
	{
		case SPA_EVT_CHARGER:
...
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT) //coriplus
				if(spa_power_iter->batt_info.health == POWER_SUPPLY_HEALTH_OVERVOLTAGE)
				{
					cancel_delayed_work_sync(&spa_power_iter->ovp_handle_work);
				}	
#endif
			...
			{ // insert case
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT) //coriplus
				ovp_charger_type = spa_power_iter->charger_info.charger_type;
				/* It should not display charging animation if it is over voltage status.
				@10.Sep.2012 gilho.park  */
				if(spa_power_iter->batt_info.health == POWER_SUPPLY_HEALTH_OVERVOLTAGE)
					break;
#endif				
				spa_update_power_supply_charger(spa_power_iter, POWER_SUPPLY_PROP_ONLINE);
		...
		case SPA_EVT_OVP:
			if(spa_power_iter->charger_info.charger_type != POWER_SUPPLY_TYPE_BATTERY)
			{
				...
				{
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT)				
					schedule_delayed_work(&spa_power_iter->ovp_handle_work, msecs_to_jiffies(200));
#else
					spa_power_iter->batt_info.health = POWER_SUPPLY_HEALTH_GOOD;
					spa_do_status(spa_power_iter, SPA_MACHINE_NORMAL, POWER_SUPPLY_STATUS_CHARGING, SPA_STATUS_NONE);
#endif
...
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT)
			else /* If the charger_type is POWER_SUPPLY_TYPE_BATTERY */
			{
				int ovp_status = (int)data;
				pr_spa_dbg(LEVEL1, "%s : SPV_EVT_OVP - %d \n", __func__, ovp_status);
				/* This status is not charging status but the battery health status 
				should be updated for next charger detection time */
				if(ovp_status == 1)
				{
					spa_power_iter->batt_info.health = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
				}
...
static int spa_init_progress(struct spa_power_desc *spa_power_iter)
{
	...
	if(spa_power_iter->init_progress <= SPA_INIT_PROGRESS_START && spa_power_iter->init_progress >= SPA_INIT_PROGRESS_DONE)
	{
...
#if defined(CHG_OVP_NO_NOTIFICATION_CONCEPT)
	INIT_DELAYED_WORK(&spa_power_iter->ovp_handle_work, spa_ovp_handle_work);
#endif
