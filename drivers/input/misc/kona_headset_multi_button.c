...
static void reset_aux_micbias(void)
{
    pr_info("%s\n", __func__);

    clk_enable(mic_dev->audioh_apb_clk);   
    if(readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VRX1_OFFSET) & 0x1)
    {
        writel(0x0, KONA_AUDIOH_VA + AUDIOH_AUDIORX_VREF_OFFSET);
        writel(0x4, KONA_AUDIOH_VA + AUDIOH_AUDIORX_VMIC_OFFSET);
        writel(0x0, KONA_AUDIOH_VA + AUDIOH_AUDIORX_BIAS_OFFSET);
    }
    else
    {
        writel(0x1, KONA_AUDIOH_VA + AUDIOH_AUDIORX_VREF_OFFSET);
        writel(0x2, KONA_AUDIOH_VA + AUDIOH_AUDIORX_VMIC_OFFSET);
        writel(0x1, KONA_AUDIOH_VA + AUDIOH_AUDIORX_BIAS_OFFSET);
    }
    writel(0x0, KONA_ACI_VA + ACI_MIC_BIAS_OFFSET);
    writel(0x1, KONA_AUXMIC_VA + AUXMIC_AUXEN_OFFSET);
    writel(0x0, KONA_AUXMIC_VA + AUXMIC_F_PWRDWN_OFFSET);
    
    pr_info("AUDIORX_VREF=0x%x, AUDIORX_VMIC=0x%x, AUDIORX_BIAS=0x%x,\nACI_MIC_BIAS=0x%x, AUXMIC_AUXEN=0x%x, AUXMIC_F_PWRDWN=0x%x\n",
             readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VREF_OFFSET), readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VMIC_OFFSET), readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_BIAS_OFFSET),
             readl(KONA_ACI_VA + ACI_MIC_BIAS_OFFSET), readl(KONA_AUXMIC_VA + AUXMIC_AUXEN_OFFSET), readl(KONA_AUXMIC_VA + AUXMIC_F_PWRDWN_OFFSET));
    clk_disable(mic_dev->audioh_apb_clk);    
}

static int check_aux_bicbias_reg(void)
{
    int ret = 1;
    int check1 = 0;
    int check2 = 0;
    ...
    if(!(!readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VREF_OFFSET) &&
        (readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VMIC_OFFSET) & 0x4) &&
        !readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_BIAS_OFFSET)))
    {
        pr_err("%s check1 set\n", __func__);
        check1 = 1;
    }
        
    if(!((readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VREF_OFFSET) & 0x1) &&
        (readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_VMIC_OFFSET) & 0x2) &&
        (readl(KONA_AUDIOH_VA + AUDIOH_AUDIORX_BIAS_OFFSET) & 0x1)))
    {
        pr_err("%s check2 set\n", __func__);
        check2 = 1;
    }
    ...
    if(!(!readl(KONA_ACI_VA + ACI_MIC_BIAS_OFFSET) &&
        (readl(KONA_AUXMIC_VA + AUXMIC_AUXEN_OFFSET) & 0x1) &&
        !readl(KONA_AUXMIC_VA + AUXMIC_F_PWRDWN_OFFSET)))
    {
        ret = 0;
        goto err;
    }
...
static int read_adc_for_accessory_detection(int hst)
{
    ...
    do
    {
         if(check_aux_bicbias_reg())
             break;

         pr_err("%s : bias registers is abnormal\n", __func__);
	...
	*/
	...
	case HEADPHONE:
		...
		else if(mic_level > HEADPHONE_DETECT_LEVEL_MAX &&
...
static void button_work_func(struct work_struct *work)
{
	...
	if(p->detect_state != DETECT_DONE)
	{
		pr_err("%s: Now DETECTING...\n", __func__);
		goto out;
	}
...
static void accessory_detect_work_func(struct work_struct *work)
{
	...
	unsigned int accessory_inserted = 0;

	p->detect_state = DETECT_ING;
   
#ifdef CONFIG_KONA_PI_MGR
    pi_mgr_qos_request_update(&qos_node, 0);
#endif

	accessory_inserted = ((gpio_get_value(irq_to_gpio(p->gpio_irq))) ^ p->headset_pd->hs_default_state);
...
static void no_gpio_accessory_insert_work_func(struct work_struct *work)
{
	...
	int loopcnt = ACCESSORY_LOOP_COUNT;   
	...
	do
	{
		...
		*/
		...
		loopcnt = (pre_comp2_level == comp2_level ? loopcnt - 1 : ACCESSORY_LOOP_COUNT);
...
irqreturn_t comp1_isr(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&(p->button_work), delay);
...
irqreturn_t comp2_inv_isr(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&(p->accessory_remove_work), ACCESSORY_INSERTION_REMOVE_SETTLE_TIME);
...
static int __init hs_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(mic->headset_dev, &headset_Attrs[STATE]);
	ret = device_create_file(mic->headset_dev, &headset_Attrs[KEY_STATE]);
	ret = device_create_file(mic->headset_dev, &headset_Attrs[RECHECK]);
	...
	mic_dev->aci_apb_clk = clk_get(&pdev->dev, "aci_apb_clk");
