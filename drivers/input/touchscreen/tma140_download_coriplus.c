...
#include "tma140_download_coriplus.h"
/* //[[ VE_GROUP_giyoung.heo_0120719_BEGIN -- .
#include "./TMA140_FIRMWARE_CORI/cori_H03S07.h"
#include "./TMA140_FIRMWARE_CORI/cori_H04S04.h"
#include "./TMA140_FIRMWARE_CORI/cori_H05S03.h"
#include "./TMA140_FIRMWARE_CORI/cori_H08S05.h"
*/ //]] VE_GROUP_giyoung.heo_0120719_END -- .

#include "./TMA140_FW/coriplus_H05S09.h"
#include "./TMA140_FW/coriplus_H08S09.h"
...
//#define ONE_MICROSSEC_CNT 100 // 312MHz	// cori plus - 100 -> 80us
#define ONE_MICROSSEC_CNT 3 // 312MHz		// cori plus - 3 -> about 2us
...
signed char fVerifySiliconID(void)
{
    ...
    {
    	printk( "[TSP] %s, %d : Error(0x%x, 0x%x)\n", __func__, __LINE__, bTargetID[0], target_id_v[0] );
        return(SiID_ERROR);
    }
    else
    {
...
unsigned int load_tma140_firmware_data(int HW_ver)
{
	int i,j;
	...
	if(tsp_special_update == 0)//normal firmware update from phone-binary
	{
		...
		*/ //]] VE_GROUP_giyoung.heo_0120719_END -- .
		...
		else if(HW_ver == 8)
		{
			for(i=0; i<512; i++)
				for(j=0; j<64; j++)
					firmData[i][j] = BinaryData_HW08[i*64 + j];
