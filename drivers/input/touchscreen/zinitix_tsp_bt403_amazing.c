...
//#include "zinitix_touch_bt4x3_firmware_R002.h"
#include "zinitix_touch_bt4x3_firmware_R002T.h"
...
static struct regulator *touch_regulator_1_8=NULL;

#define	ZINITIX_DEBUG		1

static	int	m_ts_debug_mode = ZINITIX_DEBUG;

#define	SYSTEM_MAX_X_RESOLUTION	319
#define	SYSTEM_MAX_Y_RESOLUTION	479


#if	RESET_CONTROL
#define	SYSTEM_RESET_PIN	get_gpio(0)
#endif

/* interrupt pin number*/
#define GPIO_TOUCH_PIN_NUM	91
...
// FW version
struct zinitix_touch_dev *global_touch_dev;
...
static DEVICE_ATTR(fw_update, 0777, firmware_update, firmware_update);
...
static bool ts_get_raw_data(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->raw_mode_flag == TOUCH_TEST_RAW_MODE) {
...
#if	BT4x2_Series
		if (sz < 512) {
			if (ts_read_raw_data(touch_dev->client,	ZINITIX_RAWDATA_REG, (char *)touch_dev->cur_data, sz) < 0) {
			...
			udelay(50);
			if (ts_read_raw_data(touch_dev->client,	ZINITIX_EXTRA_RAWDATA_REG,(char *)&touch_dev->cur_data[256], sz - 512) < 0) {
				printk(KERN_INFO "error : read zinitix tc raw data\n");
				up(&touch_dev->raw_data_lock);
				return false;
			}
		...
		if (!zinitix_bit_test(udata, BIT_ICON_EVENT) && !zinitix_bit_test(udata, BIT_PT_EXIST))
			memcpy((u8 *)touch_dev->ref_data, (u8 *)touch_dev->cur_data, total_node*2);

		touch_dev->update = 1;
		memcpy((u8 *)(&touch_dev->touch_info),	(u8 *)&touch_dev->cur_data[total_node],	sizeof(struct _ts_zinitix_point_info));
		up(&touch_dev->raw_data_lock);
		return true;
		...
		if (touch_dev->raw_mode_flag == TOUCH_ZINITIX_CAL_N_MODE) {
			...
			memcpy((u8 *)(&touch_dev->touch_info), (u8 *)&touch_dev->cur_data[161],	sizeof(struct _ts_zinitix_point_info));
			up(&touch_dev->raw_data_lock);
			return true;
		}
		if (sz < 512) {
		...
		if (touch_dev->raw_mode_flag == TOUCH_ZINITIX_CAL_N_MODE) {
			...
			sz = (total_cal_n*2 + sizeof(struct _ts_zinitix_point_info));
			...
			misc_touch_dev->update = 1;
			memcpy((u8 *)(&touch_dev->touch_info),	(u8 *)&touch_dev->cur_data[total_cal_n], sizeof(struct _ts_zinitix_point_info));
			up(&touch_dev->raw_data_lock);
			return true;
		}
...
static bool ts_get_samples(struct zinitix_touch_dev *touch_dev)
{
	...
	//PSJ zinitix_debug_msg("ts_get_samples+\r\n");
...
#if (TOUCH_MODE == 1)
	memset(&touch_dev->touch_info,	0x0, sizeof(struct _ts_zinitix_point_info));
	...
	if (!zinitix_bit_test(touch_dev->touch_info.status, BIT_PT_EXIST)) {
		for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
				...
				memset(&touch_dev->reported_touch_info.coord[i], 0x0, sizeof(struct _ts_zinitix_coord));
			}
		}
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
		if (zinitix_bit_test(touch_dev->touch_info.event_flag, i)) {
			udelay(20);
			if (ts_read_data(touch_dev->client, ZINITIX_POINT_STATUS_REG+2+i, (u8 *)(&touch_dev->touch_info.coord[i]), sizeof(struct _ts_zinitix_coord)) < 0) {
...
#else
	if (ts_read_data(touch_dev->client, ZINITIX_POINT_STATUS_REG, (u8 *)(&touch_dev->touch_info),	sizeof(struct _ts_zinitix_point_info)) < 0) {
		zinitix_debug_msg("error read point info using i2c.-\n");
				return false;
	}

continue_check_point_data:
	//PSJ zinitix_debug_msg("status = 0x%x , pt cnt = %d, t stamp = %d\n", touch_dev->touch_info.status,	touch_dev->touch_info.finger_cnt, touch_dev->touch_info.time_stamp);
...
{
	struct zinitix_touch_dev *touch_dev =	(struct zinitix_touch_dev *)hDevice;
	//PSJ zinitix_debug_msg("ts_read_coord+\r\n");
...
void touch_ctrl_regulator(int on_off)
{
		...
		if(touch_regulator == NULL)
	{
		...
		if(touch_regulator_1_8 == NULL)
	{
			zinitix_debug_msg(" %s, %d \n", __func__, __LINE__ );
			touch_regulator_1_8 = regulator_get(NULL, "ldo14"); 
			zinitix_debug_msg(" %s, %d \n", __func__, __LINE__ );			
			if(IS_ERR(touch_regulator_1_8)){
				zinitix_debug_msg("can not get VTOUCH_1.8V\n");
	}
			//ret = regulator_set_voltage(touch_regulator,3300000,3300000);	
			//ret = regulator_enable(touch_regulator);			
	}		
	...
	if(on_off==1)
	{
		...
		{
			zinitix_debug_msg(" %s, %d \n", __func__, __LINE__ );	
			ret = regulator_set_voltage(touch_regulator_1_8,1800000,1800000);
			zinitix_debug_msg("regulator_set_voltage ret = %d \n", ret);       			
			ret = regulator_enable(touch_regulator_1_8);
			zinitix_debug_msg("regulator_enable ret = %d \n", ret);       			
		}		
	...
	{
			ret = regulator_disable(touch_regulator_1_8);
			zinitix_debug_msg("regulator_disable ret = %d \n", ret);			
	}
...
{
	struct zinitix_touch_dev *touch_dev =	container_of(work, struct zinitix_touch_dev, tmr_work);
...
#if BT4x3_Above_Series
	if (m_firmware_data[FIRMWARE_VERSION_POS+2] == 0xff && m_firmware_data[FIRMWARE_VERSION_POS+3] == 0xff)
		return false;
	newVersion = (u16) (m_firmware_data[FIRMWARE_VERSION_POS+2] | (m_firmware_data[FIRMWARE_VERSION_POS+3]<<8));
#endif
...
u8 ts_upgrade_firmware(struct zinitix_touch_dev *touch_dev, const u8 *firmware_data, u32 size)
{
	u16 flash_addr;
	u8 *verify_data;
	int retry_cnt = 0;
	...
	do {
		...
		for (flash_addr = 0; flash_addr < size; ) {
			for (i = 0; i < TC_PAGE_SZ/TC_SECTOR_SZ; i++) {
				...
				if (ts_write_data(touch_dev->client, ZINITIX_WRITE_FLASH, (u8 *)&firmware_data[flash_addr], TC_SECTOR_SZ) < 0) {
...
retry_init:
	...
	if (i == ZINITIX_INIT_RETRY_CNT) {
		...
		pr_info("fail to check firmware data\r\n");
		goto fail_init;
	}
	...
	if (ts_write_reg(touch_dev->client,
		ZINITIX_BUTTON_SUPPORTED_NUM,
		touch_dev->cap_info.button_num) != I2C_SUCCESS)
		goto fail_init;
...
static void	zinitix_report_data(struct zinitix_touch_dev *touch_dev, int id)
{
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)	|| zinitix_bit_test(sub_status, SUB_BIT_DOWN)|| zinitix_bit_test(sub_status, SUB_BIT_MOVE)) {
			if (w == 0)
				w = 5;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (zinitix_bit_test(
		...
		/*x_lsb*/
		buf[20] =(misc_touch_dev->cur_data[total_n_num+1]&0xff);
		/*x_msb*/
		buf[21] =((misc_touch_dev->cur_data[total_n_num+1]>>8)&0xff);
		/*y_lsb*/
		buf[22] =(misc_touch_dev->cur_data[total_n_num+2]&0xff);
		/*y_msb*/
		buf[23] =((misc_touch_dev->cur_data[total_n_num+2]>>8)&0xff);
	} else {
		buf[20] = 0; /*x_lsb*/
		buf[21] = 0; /*x_msb*/
		buf[22] = 0; /*y_lsb*/
	...
	/*lsb*/
	buf[0] =(char)(misc_touch_dev->ref_data[22]&0xff);
	/*msb*/
	buf[1] =(char)((misc_touch_dev->ref_data[22]>>8)&0xff);
	/*delta lsb*/
	buf[2] =(char)(((s16)(misc_touch_dev->cur_data[22]- misc_touch_dev->ref_data[22]))&0xff);
	/*delta msb*/
	buf[3] =(char)((((s16)(misc_touch_dev->cur_data[22]- misc_touch_dev->ref_data[22]))>>8)&0xff);
	/*
	buf[4] =(char)(misc_touch_dev->ref_data[51]&0xff);
	buf[5] =(char)((misc_touch_dev->ref_data[51]>>8)&0xff);
	buf[6] =(char)(((s16)(misc_touch_dev->cur_data[51]-misc_touch_dev->ref_data[51]))&0xff);
	buf[7] =(char)((((s16)(misc_touch_dev->cur_data[51]-misc_touch_dev->ref_data[51]))>>8)&0xff);
	buf[8] =(char)(misc_touch_dev->ref_data[102]&0xff);
	buf[9] =(char)((misc_touch_dev->ref_data[102]>>8)&0xff);
	buf[10] =(char)(((s16)(misc_touch_dev->cur_data[102]-misc_touch_dev->ref_data[102]))&0xff);
	buf[11] =(char)((((s16)(misc_touch_dev->cur_data[102]-misc_touch_dev->ref_data[102]))>>8)&0xff);
	buf[12] =(char)(misc_touch_dev->ref_data[169]&0xff);
	buf[13] =(char)((misc_touch_dev->ref_data[169]>>8)&0xff);
	buf[14] =(char)(((s16)(misc_touch_dev->cur_data[169]-misc_touch_dev->ref_data[169]))&0xff);
	buf[15] =(char)((((s16)(misc_touch_dev->cur_data[169]-misc_touch_dev->ref_data[169]))>>8)&0xff);
	buf[16] =(char)(misc_touch_dev->ref_data[178]&0xff);
	buf[17] =(char)((misc_touch_dev->ref_data[178]>>8)&0xff);
	buf[18] =(char)(((s16)(misc_touch_dev->cur_data[178]-misc_touch_dev->ref_data[178]))&0xff);
	buf[19] =(char)((((s16)(misc_touch_dev->cur_data[178]- misc_touch_dev->ref_data[178]))>>8)&0xff);
	*/
...
{
	struct zinitix_touch_dev *touch_dev = global_touch_dev;    
...
#endif
{
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_DATA:
		if (copy_from_user(&m_firmware_data[2],	argp, misc_touch_dev->cap_info.chip_fw_size))
			return -1;
#if	BT4x2_Series
...
fail_hw_cal:
	...
	#if BT4x2_Series
		if (ts_write_cmd(misc_touch_dev->client, ZINITIX_SAVE_STATUS_CMD) != I2C_SUCCESS)
			goto fail_hw_cal2;
		mdelay(1000);	/* for fusing eeprom */
		if (ts_write_cmd(misc_touch_dev->client, ZINITIX_SWRESET_CMD) != I2C_SUCCESS)
			goto fail_hw_cal2;
...
fail_hw_cal2:
	...
	case TOUCH_IOCTL_SET_RAW_DATA_MODE:
		...
		if (nval == TOUCH_NORMAL_MODE && misc_touch_dev->raw_mode_flag != TOUCH_NORMAL_MODE) {
			/* enter into normal mode */
	...
	case TOUCH_IOCTL_GET_REG:
		...
		if (ts_read_data(misc_touch_dev->client, reg_ioctl.addr, (u8 *)&val, 2) < 0)
			ret = -1;
	...
	case TOUCH_IOCTL_DONOT_TOUCH_EVENT:
		...
		misc_touch_dev->work_proceedure = TS_SET_MODE;
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_INT_ENABLE_FLAG, 0) != I2C_SUCCESS)
			ret = -1;
...
static int zinitix_touch_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if (touch_dev->irq) {
		ret = request_threaded_irq(touch_dev->irq, NULL, zinitix_touch_work, IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "zinitix_ts", touch_dev);
		if (ret) {
	...
	if (device_create_file(touch_misc_device.this_device, &dev_attr_get_touch_raw_data) < 0)
...
#if USE_TEST_RAW_TH_DATA_MODE
	device_remove_file(touch_misc_device.this_device, &dev_attr_get_touch_raw_data);
...
{   
    ...
    ic_ver = g_touch_dev->cap_info.chip_firmware_version;
...
void firmware_update_test(void)
{
   int i;
	...
	misc_touch_dev->cap_info.chip_fw_size = 32*1024;
		...
		for(i=0; i < 5; i++) {
			printk("[zinitix touch] manual upgrade count = %d\r\n", i);	 
		...
		if(i >= 5) 
		{
			printk("[zinitix touch] failed to manual upgrade\r\n");	 
			return;
		} else {
			// hw calibration and make checksum
