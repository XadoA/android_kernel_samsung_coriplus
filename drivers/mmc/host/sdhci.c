...
//		printk("%s: CMD:[%d]  SDHC_PRESENT_STATE=0x%x; Command Arg=0x%x\n",
//				mmc_hostname(host->mmc), cmd->opcode,
//				sdhci_readl(host, SDHCI_PRESENT_STATE), cmd->arg);
...
static void sdhci_work_wait_erase(struct work_struct *work)
{
	...
	/*
	According to Arasan, when Data Timeout Error is occured during sending CMD38
	,then SD Host Controller recognizes the Data Timeout Error as normal error. 
	So sd driver should reset the DATA Line for SDHC internal state.
	*/
	printk("%s-There is DAT Timeout during CMD39-->Reset DAT Line\n",__func__);
	sdhci_reset(host, SDHCI_RESET_DATA);
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (intmask & SDHCI_INT_TIMEOUT)	{
		host->cmd->error = -ETIMEDOUT;
		if(host->cmd->opcode == 13 || host->cmd->opcode == 18 ||
			host->cmd->opcode == 17 || host->cmd->opcode == 25 ||
			host->cmd->opcode == 24)	{
			pr_err("%s: CMD%d got timeout!!!\nSDHCI Register Dump:\n",
...
//			printk("CMDComplete Interrupt is done for CMD30 and wait for DATComplete Interrupt\n");
		
		if (host->cmd->data)
			pr_err("Cannot wait for busy signal when also "
...
//				if(host->cmd->opcode == MMC_ERASE)
//					printk("DATComplete Interrupt is done for CMD38, not Data Timeout Error\n");
		...
		/* If spurious data interrupt when there is no mmc request,
		 * If host->mrq may not be set. Warn and return in this case
		 */
		WARN_ON(host->mrq == NULL);
		if(!host->mrq)
			return;
		...
		if(host->mrq->data){
			if(host->mrq->data->error){
				printk(KERN_ERR"%s- It is possible that the damaged SD card is inserted - Error:0x%x\n"
									,mmc_hostname(host->mmc),host->mrq->data->error);           
				host->mrq->cmd->error = -ENOMEDIUM;
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	if((intmask & SDHCI_INT_TIMEOUT) && host->cmd && (host->cmd->opcode == 13 ||
		host->cmd->opcode == 18 || host->cmd->opcode == 17 ||
		host->cmd->opcode == 25 || host->cmd->opcode == 24))	{
		pr_err("****%s:%s: intmask=0x%x\n", __func__,
			mmc_hostname(host->mmc), intmask);
		}
