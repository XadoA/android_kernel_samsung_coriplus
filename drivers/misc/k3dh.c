...
// Average Filter for noise by vibrator (only coriplus)
#if defined(CONFIG_MACH_RHEA_SS_CORIPLUS)
#define K2DM_AVERAGE_FILTER
#endif
...
#if defined(K2DM_AVERAGE_FILTER)
	struct work_struct work_accel_avg;
	struct hrtimer accel_avg_timer;
	struct workqueue_struct *wq_avg;
	ktime_t accel_polling_time;
	int avg[3];
	int accel_avg_enable;
#endif
};
...
{    
	...
	int err = -1;
#if defined(K2DM_AVERAGE_FILTER)
	if (g_k3dh->accel_avg_enable == 1)
	{
		xyz[0] = g_k3dh->avg[0];
		xyz[1] = g_k3dh->avg[1];
		xyz[2] = g_k3dh->avg[2];
		ACCDBG("[K3DH][AVG] x:%d, y:%d, z:%d\n", xyz[0], xyz[1], xyz[2]);
		err =0;
	}
	else
#endif
...
void accsns_activate(int flgatm, int flg, int dtime)
{
    ...
    if (flg == 0) //sleep
    {
		...
		if( g_k3dh->accel_avg_enable == 1)
		{
			printk("[%s] cancelling accel avg poll timer\n", __func__);
			hrtimer_cancel(&g_k3dh->accel_avg_timer);
			cancel_work_sync(&g_k3dh->work_accel_avg);
			g_k3dh->accel_avg_enable = 0;
		}
#endif
    }
    else 
    {
		...
		if(dtime == 200)
		{
			if (g_k3dh->accel_avg_enable == 0)
			{
				printk("[%s] starting poll timer, delay %lldns\n",__func__,
				ktime_to_ns(g_k3dh->accel_polling_time));
				hrtimer_start(&g_k3dh->accel_avg_timer,
					g_k3dh->accel_polling_time, HRTIMER_MODE_REL);
				g_k3dh->accel_avg_enable = 1;
			}
		}
		else if (g_k3dh->accel_avg_enable == 1)
		{
			printk("[%s] cancelling accel avg poll timer\n", __func__);
			hrtimer_cancel(&g_k3dh->accel_avg_timer);
			cancel_work_sync(&g_k3dh->work_accel_avg);
...
#if defined(K2DM_AVERAGE_FILTER)
int k3dh_get_acceleration_avg_data(int *xyz)
{
	struct k3dh_acc acc;
	int err = -1;
	static int avg_buf[10][3] =  {0,};
	static int avg_idx = 0;
	static int avg_cnt = 0;
	int sum_xyz[3] = {0,};
	int min_xyz[3] = {512, 512, 512};
	int max_xyz[3] = {-512, -512, -512};
	int avg_len = 10;
	int i, j = 0;

	err = k3dh_read_accel_xyz(&acc);

	avg_buf[avg_idx][0] = ((int)(acc.x)*4);
	avg_buf[avg_idx][1] = ((int)(acc.y)*4);
	avg_buf[avg_idx][2] = ((int)(acc.z)*4);

	avg_idx = ((avg_idx + 1) % avg_len);
	if (avg_cnt < avg_len) avg_cnt++;

	for (i = 0; i< avg_cnt; i++) {
		for (j = 0; j< 3; j++) {
			sum_xyz[j] += avg_buf[i][j];
			if(avg_buf[i][j] < min_xyz[j])
				min_xyz[j] = avg_buf[i][j];
			if(avg_buf[i][j] > max_xyz[j])
				max_xyz[j] = avg_buf[i][j];
		}
	}
	...
	if (avg_cnt > 0) {
		for (j = 0; j< 3; j++)
			if(avg_cnt > 2)
				xyz[j] = (sum_xyz[j] - max_xyz[j] - min_xyz[j]) / (avg_cnt-2);
			else
				xyz[j] = sum_xyz[j] / avg_cnt;
	}

	return err;
}

static void accel_work_func_avg(struct work_struct *work)
{
	int acc_avg[3] = {0,};
	int err = -1;

	err = k3dh_get_acceleration_avg_data(&acc_avg);
	if(err < 0)
                pr_err("%s: get accel avg data error : %d\n", __func__, err);
	g_k3dh->avg[0] = acc_avg[0];
	g_k3dh->avg[1] = acc_avg[1];
	g_k3dh->avg[2] = acc_avg[2];
}

static enum hrtimer_restart accel_timer_func(struct hrtimer *timer)
{
	queue_work(g_k3dh->wq_avg, &g_k3dh->work_accel_avg);
	hrtimer_forward_now(&g_k3dh->accel_avg_timer, g_k3dh->accel_polling_time);
...
static DEVICE_ATTR(calibration, S_IRUGO | S_IRUSR | S_IROTH, k3dh_calibration_show, NULL);
static DEVICE_ATTR(raw_data, S_IRUGO | S_IRUSR | S_IROTH, k3dh_fs_read, NULL);
...
static int k3dh_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if defined(K2DM_AVERAGE_FILTER)
	g_k3dh->wq_avg = create_singlethread_workqueue("accel_wq_avg");
	if (!g_k3dh->wq_avg) {
		err = -ENOMEM;
	...
	INIT_WORK(&g_k3dh->work_accel_avg, accel_work_func_avg);
	g_k3dh->accel_avg_enable = 0;
	g_k3dh->avg[0] = 0;
	g_k3dh->avg[1] = 0;
	g_k3dh->avg[2] = 0;

	hrtimer_init(&g_k3dh->accel_avg_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	g_k3dh->accel_polling_time = ns_to_ktime(10 * NSEC_PER_MSEC);
	g_k3dh->accel_avg_timer.function = accel_timer_func;
#endif
