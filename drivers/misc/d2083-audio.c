...
// TODO: WILLIE - Forced proper gain(preamp:-2dB, Amp:-6dB). This is for bring-up.
...
static inline int d2083_audio_poweron(bool on)
{
	...
	if(on==true)		
		audio_write(D2083_LDO_AUD_MCTL_REG, 0x44);
...
int d2083_audio_hs_poweron1(bool on)
{
    ...
    dlg_info("d2083_audio_hs_poweron1 on=%d speaker_power=%d \n", on,dlg_audio->IHFenabled);     
    ...
    if(on) 
	{
		...
		ret |= audio_write(D2083_MXHPR_CTRL_REG,0x07);
		...
		dlg_info("hs_pre_gain on=0x%x hs_pga_gain=0x%x \n", 
					dlg_audio->hs_pre_gain,dlg_audio->hs_pga_gain);     
...
int d2083_audio_hs_ihf_poweron1(void)
{
	...
	dlg_info("d2083_audio_hs_ihf_poweron  headset_power=%d \n", dlg_audio->HSenabled);
	...
	ret |= audio_set_bits(D2083_PREAMP_A_CTRL1_REG, D2083_PREAMP_EN);
	ret |= audio_clear_bits(D2083_PREAMP_A_CTRL1_REG, D2083_PREAMP_MUTE);

	ret |= audio_write(D2083_MXSP_CTRL_REG,0x07);//Mixer set to A1+A2
	ret |= audio_set_bits(D2083_SP_CTRL_REG, D2083_SP_EN);
	ret |= audio_clear_bits(D2083_SP_CTRL_REG, D2083_SP_MUTE);
	...
	ret |= audio_write(D2083_BIAS_CTRL_REG,0x06); 
...
int d2083_audio_hs_ihf_poweroff(void)
{
	...
	dlg_info("d2083_audio_hs_ihf_poweroff  headset_power=%d \n", dlg_audio->HSenabled);

	audio_write(D2083_SP_CTRL_REG,0x32);
	audio_write(D2083_MXSP_CTRL_REG,0x00);
	audio_write(D2083_PREAMP_A_CTRL1_REG,0x32);    
...
int d2083_audio_multicast(bool on)
{
	u8 regval;
	int ret = 0;
	
	dlg_info("d2083_audio_multicast = %d \n", on);
	...
	if(on){
		...
		regval = audio_read(D2083_PREAMP_A_CTRL1_REG) & ~D2083_PREAMP_VOL;
		regval |= (D2083_PREAMP_GAIN_0DB << D2083_PREAMP_VOL_SHIFT) & D2083_PREAMP_VOL;
		
		ret |= audio_write(D2083_PREAMP_A_CTRL1_REG,regval);
		
		ret |= audio_write(D2083_MXSP_CTRL_REG,0xBF);//Mixer set to A+B
		ret |= audio_write(D2083_SP_CTRL_REG,
			((D2083_SPVOL_4DB << D2083_SP_VOL_SHIFT)|D2083_SP_EN));
		...
		ret |= audio_write(D2083_SP_CTRL_REG,0x32);
		ret |= audio_write(D2083_MXSP_CTRL_REG,0x00);
		ret |= audio_write(D2083_PREAMP_A_CTRL1_REG,0x32);	  
	}
...
static int d2083_audio_probe(struct platform_device *pdev)
{
	...
	audio_write(D2083_CP_CTRL_REG, 0xCD);
