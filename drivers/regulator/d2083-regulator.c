...
void touch_ldo_onoff(int onoff)
{
	if(d2083_regl_info==NULL){
		dlg_err("error : touch_ldo_onoff\n");
		return;
	}

	if(onoff) {	// 1: on
		dlg_info("touch_ldo_on\n");
		d2083_reg_write(d2083_regl_info, D2083_LDO14_MCTL_REG, 0x44);
		d2083_reg_write(d2083_regl_info, D2083_LDO15_MCTL_REG, 0x44);
	} else {			// 0: off
		dlg_info("touch_ldo_off\n");
		d2083_reg_write(d2083_regl_info, D2083_LDO14_MCTL_REG, 0x00);
		d2083_reg_write(d2083_regl_info, D2083_LDO15_MCTL_REG, 0x00);
	}
}
EXPORT_SYMBOL(touch_ldo_onoff);
#endif
...
static int d2083_regulator_set_voltage(struct regulator_dev *rdev, int min_uV, int max_uV,unsigned *selector)
{
	...
	*selector = -1;
	...
	printk(KERN_ERR "[WS] regl_id[%d], reg_num[0x%x], reg_val[0x%x]\n", 
				regulator_id, reg_num, (val | mV_val));
...
static int d2083_regulator_enable(struct regulator_dev *rdev)
{
	...
	if(!is_mode_control_enabled()){
		...
		reg_val &= ~(D2083_REGULATOR_MCTL1 | D2083_REGULATOR_MCTL3);   // Clear MCTL11 and MCTL01
		reg_val |= (D2083_REGULATOR_MCTL1_ON | D2083_REGULATOR_MCTL3_ON);

		switch(get_regulator_dsm_opmode(regulator_id)) {
		case D2083_REGULATOR_LPM_IN_DSM :
			reg_val &= ~(D2083_REGULATOR_MCTL0 | D2083_REGULATOR_MCTL2);
			reg_val |= (D2083_REGULATOR_MCTL0_SLEEP | D2083_REGULATOR_MCTL2_SLEEP);
			break;
		...
		case D2083_REGULATOR_ON_IN_DSM :
			reg_val &= ~(D2083_REGULATOR_MCTL0 | D2083_REGULATOR_MCTL2);
			reg_val |= (D2083_REGULATOR_MCTL0_ON | D2083_REGULATOR_MCTL2_ON);
			break;
		}
...
static int d2083_regulator_set_mode(struct regulator_dev *rdev, unsigned int mode)
{
	...
	ret = set_regulator_mctl_mode(d2083, regulator_id, mode);
...
#if defined(CONFIG_MFD_D2083_BRINGUP_RECHECK)	// temporarily
extern int bcm_hsotgctrl_en_clock(bool on);
extern int bcm_hsotgctrl_set_bc_iso(bool on);
#endif
