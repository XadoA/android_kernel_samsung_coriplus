...
#endif	
printk("USBD] send_usb_event ievent=%d\n",ievent);
...
{
//printk("USBD] send_usb_insert_event\n");
//printk("USBD]  send_usb_insert_eventcore_if=0x%x core_if->wq_otg=0x%x\n",core_if,core_if->wq_otg);
	//	DWC_WORKQ_SCHEDULE(core_if->wq_otg, w_init_core,
	//	   core_if, "init core");
enum bcmpmu_usb_type_t usb_type = PMU_CHRGR_TYPE_SDP;
printk("USBD] send_usb_attach_event\n");
...
printk("USBD] send_usb_detach_event\n");
...
printk("USBD] fsa9480_otg_xceiv_chg_detection_notif_handler\n");
	if (!xceiv_data || !data)
		return -EINVAL;
...
printk("USBD] fsa9480_otg_xceiv_chg_detect_handler\n");
	if (xceiv_data->otg_enabled) {
		printk("USBD] fsa9480_otg_xceiv_chg_detect_handler\n");
		...
		bool id_default_host = false;
printk("USBD] fsa9480_otg_xceiv_chg_detect_handler 2\n");
		id_default_host =
			fsa9480_otg_xceiv_check_id_gnd(xceiv_data) ||
			fsa9480_otg_xceiv_check_id_rid_a(xceiv_data);

		if (!id_default_host && xceiv_data->otg_xceiver.xceiver.gadget){
			printk("USBD] fsa9480_otg_xceiv_chg_detect_handler 3\n");
			atomic_notifier_call_chain(&xceiv_data->otg_xceiver.xceiver.notifier,
						   USB_EVENT_VBUS, NULL);// then this calls dwc_otg_xeiv_nb_callback().
			}
	}
}
...
printk("USBD] fsa9480_otg_xceiv_probe\n");
...
//dh0318.lee
	printk("%s-initializes fsa9480_event with event id",__func__);
...
//	xceiv_data->bcmpmu = bcmpmu;
	...
	printk("%s, initalize ATOMIC Notifier header:0x%x\n"
		,__func__,&xceiv_data->otg_xceiver.xceiver.notifier);
	...
	*/
	...
	printk("%s, register notifier for mUIC Event to fsa9480 driver\n",__func__);
	...
	*/
	...
	printk("%s, complete to register notifier for mUIC Event to fsa9480 driver\n",__func__);
