...
static Boolean stereoOutputTriggered = FALSE;
...
/*============================================================================
//
// Function Name: void csl_caph_audioh_start(int path_id, Boolean isDitherOff, Boolean stereoOutput)
//
// Description:  Start playback or recording on path "path_id" on CSL layer,
//               the path is already configured
//
// Parameters:   path_id : audio path
//               isDitherOff : check if dither should be off
//               stereoOutput : check if it is stereo output
//
// Return:
//
//===========================================================================*/
void csl_caph_audioh_start(int path_id, Boolean isDitherOff, Boolean stereoOutput)
#else
/*============================================================================
//
// Function Name: void csl_caph_audioh_start(int path_id, Boolean isDitherOff)
//
// Description:  Start playback or recording on path "path_id" on CSL layer,
//               the path is already configured
//
...
#endif
{
	...
	if (path_id > 0 && path_id < AUDDRV_PATH_TOTAL) {
		if (path_id == AUDDRV_PATH_HEADSET_OUTPUT) {
			if (!isDitherOff) {
                        /*enable dither*/
                        chal_audio_hspath_sdm_set_dither_seed(handle,1,1);
                        chal_audio_hspath_sdm_set_dither_poly(handle,0x48000000,0x41000000);
                        chal_audio_hspath_sdm_set_dither_strength(handle,1,1);
	...
	aTrace
	      (LOG_AUDIO_CNTLR, "csl_caph_audioh_start:: %d, isDitherOff %d.\r\n", path_id, isDitherOff);
	switch (path_id) {
	case AUDDRV_PATH_VIBRA_OUTPUT:
...
#if defined(CONFIG_STEREO_SPEAKER)
		aTrace
		      (LOG_AUDIO_CNTLR, "csl_caph_audioh_start:: stereoOutput %d.\r\n", stereoOutput);

		stereoOutputTriggered = stereoOutput;
		if (stereoOutput) {
			/* disable ramp for earpiece output when it's used for stereo output */
			chal_audio_earpath_ramp_ctrl(handle, 0);
		} else {
			/* enable ramp for earpiece output when it's used for real earpiece (mono) output */
			chal_audio_earpath_ramp_ctrl(handle, 1);
		}
#endif
...
#if defined(CONFIG_STEREO_SPEAKER)
		aTrace
		      (LOG_AUDIO_CNTLR, "csl_caph_audioh_stop_keep_config:: stereoOutputTriggered %d.\r\n", stereoOutputTriggered);

		if (!stereoOutputTriggered) {
			/* re-enable ramp for earpiece output when it's used for real earpiece (mono) output */
			chal_audio_earpath_ramp_ctrl(handle, 1);
		}
#endif
...
*
*  Parameters:  isDitherOff : check if dither should be off
*
****************************************************************************/
void csl_caph_audioh_start_hs(Boolean isDitherOff)
{
	...
	aTrace(LOG_AUDIO_CNTLR, "%s, audioh_hs_on %d, isDitherOff %d\n", __func__, audioh_hs_on, isDitherOff);
