...
struct regulator_consumer_supply d2083_ldo9_supplies[] = {
	{.supply = "vdd_sdxc"},
};

struct regulator_consumer_supply d2083_ldo13_supplies[] = {
	{.supply = "vdd_sdio"},
};
#endif
...
static struct i2c_board_info __initdata musb_i2c_devices[] = {
	{
				I2C_BOARD_INFO("fsa9480", 0x4A>>1),
				.irq = gpio_to_irq(MUSB_INT),
	},
};
#endif
...
#if defined(CONFIG_STEREO_SPEAKER)// NEVIS_AMP
...
static int rhea_camera_power_sub(struct device *dev, int on)
{
	...
	printk(KERN_INFO "rhea_camera_power_sub %d %d\n", on, unicam_dfs_node.valid);
...
i2c_register_board_info(0x3, zinitix_i2c_devices,
				ARRAY_SIZE(zinitix_i2c_devices)); //PSJ[temp]
...
#endif
#if 1
i2c_register_board_info(0x8, musb_i2c_devices,
				ARRAY_SIZE(musb_i2c_devices));
#endif
}
...
#ifdef CONFIG_MACH_RHEA_SS_NEVISP
MACHINE_START(RHEA, "rhea_ss_nevisp")
#else
MACHINE_START(RHEA, "rhea_ss_nevis")
#endif
	.map_io = board_map_io,
