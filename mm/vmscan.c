		...
		unsigned long *scanned, int order, int mode, int file)
{
	...
	for (scan = 0; scan < (nr_to_scan + nr_skipped) &&
		...
		default:
			printk(KERN_EMERG"__isolate_lru_page failed !!!!\n");
			printk(KERN_EMERG"== isolate_lru_pages Page Dump ==\n");
			dump_page(page);
			printk(KERN_EMERG"=================================\n");
			printk(KERN_EMERG"page(0x%08x) pfn(%lx) prev(0x%08x)"
					" mode(%d) file(%d) order(%d)"
					" isolate_cma(%d)\n",
					page, page_to_pfn(page), prev, mode,
					file, order, isolate_cma);
			printk(KERN_EMERG"nr_to_scan(%lu) scan(%lu)"
					" nr_skipped(%lu) phys_page(0x%08x)\n",
					nr_to_scan, scan, nr_skipped,
				     ...
				     enum lru_list lru)
{
	...
	while (!list_empty(list)) {
		...
		BUG_ON(!is_active_lru(lru) != !PageActive(page));
...
retry:
	...
	if (page_evictable(page, NULL)) {
		...
		BUG_ON(!is_active_lru(l) != !PageActive(page));
